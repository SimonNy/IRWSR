...
def findAndMatch(img1, img2, nOrbs, nMatches, displayImages = True):
...
def findOpticalFlow(img1, img2, displayImages = True):
    ...
    save_folder = "imageRegistration/"
    test1 = np.copy(img1)
    test2 = np.copy(img2)
    # plt.imsave(save_folder+"img1.png", img1, cmap="gray")
    # plt.imsave(save_folder+"img2.png", img2, cmap="gray")
    img2 = np.copy(test1)
    img2[:, 450:] = test2[:, 450:]
    img2[:, 430:470] = 255
    # plt.imshow(crop, cmap="gray")
    # plt.imsave(save_folder+"transCrop.png",
    # crop, cmap="gray", vmin=0, vmax=255)
    


    test1 = cv2.cvtColor(test1, cv2.COLOR_GRAY2RGB)
    test2 = cv2.cvtColor(test2, cv2.COLOR_GRAY2RGB)
    for i in p1:
        x, y = i.ravel()
        cv2.circle(test1, (x, y), 3, 255, -1)
    for i in p2:
        x2, y2 = i.ravel()
        cv2.circle(test2, (x2, y2), 3, 255, -1)
    
    plt.imsave(save_folder+"edges1.png", test1, cmap="gray")
    plt.imsave(save_folder+"edges2.png", test2, cmap="gray")

    crop = np.copy(test1)
    crop[:, 450:] = test2[:, 450:]
    crop[:, 430:470] = 255
    plt.imshow(crop, cmap="gray")
    plt.imsave(save_folder+"edgeCrop.png",
               crop, cmap="gray", vmin=0, vmax=255)

    plt.imsave(save_folder+"thres1.png", gray1, cmap="gray")
    plt.imsave(save_folder+"thres2.png", gray2, cmap="gray")
    crop = np.copy(gray1)
    crop[:, 450:] = gray2[:, 450:]
    crop[:, 430:470] = 255
    plt.imshow(crop, cmap="gray")
    plt.imsave(save_folder+"thresCrop.png",
               crop, cmap="gray", vmin=0, vmax=255)
    ...
    if displayImages:
        ...
        plt.imshow(output)
        plt.imsave(save_folder+"opticalFlow.png", output, cmap="gray")
        plt.show()
